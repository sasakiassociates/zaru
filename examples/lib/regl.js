...
function reglCore (
  ...
  function parseProgram (options) {
    ...
    function parseShader (name) {
      if (name in staticOptions) {
        ...
        check$1.optional(function () {
          shaderState.vertexShader(shaderType[name], id, check$1.guessCommand());
        });
        ...
        return createDynamicDecl(dyn, function (env, scope) {
          ...
          check$1.optional(function () {
            scope(
              env.shared.vertexShader, '.shader(',
              shaderType[name], ',',
              id, ',',
              env.command, ');');
          });
    ...
    if (isStatic(frag) && isStatic(vert)) {
      ...
      progVar = new Declaration(
        ...
        function (env, scope) {
          var SHADER_STATE = env.shared.vertexShader;
          var fragId;
          if (frag) {
  ...
  function emitDrawProc (env, args) {
    ...
    var program = args.vertexShader.progVar.append(env, draw);
    draw(env.shared.gl, '.useProgram(', program, '.program);');

    if (args.vertexShader.program) {
      emitDrawBody(env, draw, args, args.vertexShader.program);
    } else {
  ...
  function emitBatchBody (env, scope, args, program) {
    ...
    if (!program) {
      ...
      var PROGRAM = args.vertexShader.progVar.append(env, inner);
  ...
  function emitBatchProc (env, args) {
    ...
    var progDefn = args.vertexShader.progVar;
  ...
  function emitScopeProc (env, args) {
    ...
    function saveShader (name) {
      var shader = args.vertexShader[name];
      if (shader) {
        scope.set(shared.vertexShader, '.' + name, shader.append(env, scope));
      }
    }
    saveShader(S_VERT);
    saveShader(S_FRAG);
