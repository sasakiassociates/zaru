...
L.TileLayer.Regl = L.GridLayer.extend({
    ...
    initialize: function (options) {
        ...
        this._renderer = L.DomUtil.create('canvas', 'regl-canvas-tile');
        ...
        for (let i = 0; i < this.options.tileUrls.length && i < MAX_SHADERS; i++) {
...
//			attribute vec2 aTextureCoords;
    ...
    getTileRects: function (latlng, size) {
        ...
        if (this.options.limits) {
            const limits = this.options.limits[0];//TODO support multiple tiles of different limits...
            limitZoom = limits.zoom;
        }
        ...
        if (this.options.tileUrls.length === 0) {
            ...
            for (let i = 0; i < this.options.tileUrls.length && i < MAX_SHADERS; i++) {
                ...
                if (tileSource.canvasRender) {
                    ...
                    if (!cr.canvasSets) {
                        // console.log('GENERATING canvasSets (should happen once only)');
                        cr.canvasSets = {};//use a centralized cache based on the canvasRenderer being shared between tiles
                    }

                    if (!cr.canvasSets[tileId]) {
                        ...
                        const size = cr.canvasSize || {width: this.options.tileSize, height: this.options.tileSize};

                        canvasTile.width = size.width;
                        canvasTile.height = size.height;
                        cr.canvasSets[tileId] = {
                            canvas: canvasTile,
                            coords: coords,
                            cache: {},
                            images: {},
                    ...
                    const canvasSet = cr.canvasSets[tileId];

                    //canvas render supports loading a bunch of slippy map tiles and compositing them on canvas or
                    //simply providing access to a canvas element that can be rendered from another non-tile data source
                    if (cr.urls && cr.urls.length > 0) {
                    ...
                    texFetches.push(new Promise(function (resolve, reject) {//Not a real 'fetch' but treat the same for the promises handling
                        const getTex = () => {
                            if (canvasSet.needsUpdate) {
                                // console.log('REGL NEW canvas texture ' + tileId);
                                canvasSet.texture = regl.texture(canvasSet.canvas);
                                canvasSet.needsUpdate = false;//will update when
                            }
                            return canvasSet.texture;
                        };
                        resolve({
                            image: canvasSet.canvas,
                            texture: getTex,
                            textureCoords: textureCoords
                        });
                    }));
